                ...
                "changes": [
                    {
                        ...
                        "items": [
                            ...
                            "Выпадающий блок «Требуемые колонки» для каждого шага",
                            "Поддержка subFiles для шага «Активные пользователи» (прошлый/текущий месяц)",
                            "Кликабельный индикатор шагов с тултипами",
                            "Модальное окно подтверждения при сбросе данных",
                            "Интерактивный refresh токена через popup (fallback для silent refresh)"
                        ]
                    },
                    {
                        "category": "Улучшено",
                        "items": [
                            "Страница логина: полный редизайн Dark Glassmorphism (градиенты, blur, тени)",
                            "Свободная навигация по шагам (как в расчёте трафика)",
                            "Премиальный редизайн выбора шаблонов с выпадающим списком компонентов",
                            "Компактный UI для subFiles (сетка из двух столбцов)",
                            "SVG иконки вместо эмодзи в списке компонентов",
                            "Унифицированный стиль кнопок загрузки файлов",
                            "Увеличено количество попыток обновления токена до 5 (было 3)",
                            "Рефакторинг login.js (упрощение кода)"
                        ]
                    },
                    {
                        "category": "Изменено",
                        "items": [
                            "Общий отчёт: 6 шагов вместо 7 (Активные пользователи объединены в один шаг)",
                            "B-TAG: id изменён на b-tag (унификация идентификаторов)",
                            "Подтверждение перед редиректом на логин при истечении токена"
                        ]
                    },
                    {
                        "category": "Исправлено",
                        "items": [
                            "Кнопка «Начать заново» работает корректно",
                            "Очистка данных при возврате к предыдущему шагу",
                            "Навигация между шагами в Excel Reports",
                            "Работа кнопки «Из чего состоит»",
                            "Silent refresh: добавлено корректное закрытие popup окна",
                            "CSP: добавлен accounts.google.com в script-src на всех страницах"
                        ]
                    },
                    {
                        "category": "Файлы",
                        "items": [
                            ...
                            "excel-reports/templates/*.js (requiredColumns, subFiles)",
                            "login/css/login.css, login/js/login.js, login/callback.html",
                            "shared/auth-guard.js (interactive refresh)",
                            "CSP обновлён: index, partners, documentation, sync, team-info, traffic-calculation"
                        ]
                    }
                ]
            },
