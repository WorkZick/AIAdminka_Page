...
(function() {
    ...
    // Popup refresh: открыт из extendSession() через window.open
    const isPopupRefresh = sessionStorage.getItem('oauth_popup_refresh') === 'true';
    // Определяем, что мы в popup (window.opener существует)
    ...
    function handleCallbackError(e) {
        ...
        if (isPopupRefresh && isInPopup) {
            // Popup refresh не удался — сообщаем opener, НЕ удаляем токен
        ...
        // При ошибке во время redirect refresh — НЕ удаляем токен,
        // возвращаем пользователя на страницу (токен ещё может быть действителен).
        if (isRedirectRefresh) {
            sessionStorage.setItem('oauth_needs_interactive', 'true');
    ...
    try {
        ...
        if (error) {
            ...
            if (isPopupRefresh && isInPopup) {
                // prompt=none не сработал в popup — НЕ удаляем токен, сообщаем opener
                sessionStorage.removeItem('oauth_popup_refresh');
                window.opener.postMessage({
                    type: 'oauth-token-refresh',
                    success: false,
            ...
            if (isRedirectRefresh) {
                // prompt=none не сработал — вернуть на страницу, НЕ удаляя токен
                // Persist флаг для interactive — следующая попытка будет с prompt=consent
                sessionStorage.setItem('oauth_needs_interactive', 'true');
                sessionStorage.removeItem('oauth_redirect_refresh');
                const returnUrl = sessionStorage.getItem('auth-redirect');
                sessionStorage.removeItem('auth-redirect');
        ...
        .then(function(userInfo) {
            ...
            if (isPopupRefresh && isInPopup) {
                // Popup refresh успешен — отправляем токен в opener
                sessionStorage.removeItem('oauth_popup_refresh');
                window.opener.postMessage({
                    type: 'oauth-token-refresh',
                    success: true,
