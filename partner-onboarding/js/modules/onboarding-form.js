...
const OnboardingForm = (() => {
    ...
    function _shouldShowField(field, stepData, step) {
        ...
        if (field.showWhen.phase === 'fill' && step.dynamicExecutor) {
            ...
            // AutoHandoff: reviewer fills directly (treat as if creator === reviewerValue)
            if (step.dynamicExecutor.autoHandoff) {
                const ahRole = OnboardingState.get('userRole');
                const ahSysRole = OnboardingState.get('systemRole');
    ...
    function render(request, stepNumber) {
        ...
        // Past or future step preview: readonly, hide banner + submit
        ...
        // Step 5: dynamically update button text + comment required on antifraud_result change
        if (stepNumber === 5) {
    ...
    function _renderFields(step, data, request) {
        ...
        const isPastOrFuturePreview = request && step.number !== request.currentStep;

        if (isPastOrFuturePreview) {
            const html = step.fields.map(field => {
                if (field.asSubmitButton) return '';
                // Hide showWhen fields with no data
                if (field.showWhen && field.showWhen.phase === 'fill') {
                    const v = data[field.id];
                    if (v === undefined || v === '' || v === null) return '';
                }
                // Hide visibleWhen fields where condition doesn't match
                if (field.visibleWhen && data[field.visibleWhen.field] !== field.visibleWhen.value) return '';
        ...
        // AutoHandoff: executor sees waiting state when reviewer hasn't filled yet
        if (step.dynamicExecutor && step.dynamicExecutor.autoHandoff && !stepData._handoff_complete) {
            ...
            if (!OnboardingRoles.isReviewerForStep(formSysRole, step.number) &&
                OnboardingState.get('userRole') !== 'admin' && OnboardingState.get('userRole') !== 'leader') {
                container.innerHTML = `<div class="executor-completed-banner">
                    <img src="../shared/icons/settings.svg" width="48" height="48" alt="" class="completed-icon">
                    <h3 class="completed-title">Ожидание создания</h3>
                    <p class="completed-subtitle">${Utils.escapeHtml(step.name)}</p>
                    <p class="completed-note">Ревьюер создаёт аккаунт корп. мессенджера. После создания вы получите данные для подтверждения.</p>
                </div>`;
                return;
            }
        }
    ...
    function _renderReadonlyPreview(field, value) {
        ...
        if (field.type === 'checklist') {
            if (typeof value !== 'object' || value === null) {
                return _readonlyField(field.label, '—');
            }
            const hasAnyChecked = (field.items || []).some((_, idx) => value[idx]);
            if (!hasAnyChecked) return _readonlyField(field.label, '—');
        ...
        if (field.type === 'file') {
            if (field.multiple) {
                ...
                if (!urls.length) return _readonlyField(field.label, '—');
                return `<div class="readonly-field">
                    <span class="readonly-label">${Utils.escapeHtml(field.label)}</span>
            ...
            if (empty) return _readonlyField(field.label, '—');
        ...
        if (field.type === 'list') {
            if (!Array.isArray(value) || !value.length) return _readonlyField(field.label, '—');
            return _readonlyField(field.label, value.map(v => Utils.escapeHtml(v)).join(', '));
        }
        ...
        return _readonlyField(field.label, Utils.escapeHtml(String(display)));
    }

    function _readonlyField(label, value) {
        return `<div class="readonly-field">
    ...
    function _updateSubmitButton(step, stepNumber) {
        ...
        // Future step — show "К текущему шагу"
        if (request && stepNumber > request.currentStep) {
            if (statusWrap) statusWrap.classList.add('hidden');
            btn.classList.remove('hidden');
            btn.textContent = 'К текущему шагу';
            btn.setAttribute('data-action', 'onb-goToCurrentStep');
            return;
        }

        // Restore default action if it was changed
        btn.setAttribute('data-action', 'onb-submit');
        ...
        if (OnboardingConfig.isExecutorFinalStep(stepNumber) || stepNumber === OnboardingConfig.STEPS.length) {
        ...
        // Step 5 (Antifraud): override button text when result is "failed"
        if (stepNumber === 5 && request) {
    ...
    function _handleFileChange(e) {
        ...
        if (input.multiple) {
            ...
            if (!Array.isArray(_fileDataUrls[fieldId])) {
                _fileDataUrls[fieldId] = Array.isArray(_fileDataUrls[fieldId]) ? _fileDataUrls[fieldId] : [];
            }
    ...
    function validate(stepNumber) {
        ...
        // Step 5: antifraud_comment required when result is "failed"
        if (stepNumber === 5 && merged.antifraud_result === 'failed') {
    ...
    function removeMultiFile(fieldId, index) {
        ...
        if (container) {
            ...
            container.innerHTML = allUrls.map((u, i) => `<div class="file-preview" data-index="${i}">
                <img src="${u.startsWith('data:') ? u : Utils.escapeHtml(u)}" alt="" data-action="onb-openPhoto" data-value="${fieldId}">
